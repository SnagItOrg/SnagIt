...
/**
 * INTEGRATION. WP-4a authored these five unions as literal restatements of
 * WP-5's, kept in step by the comments above them, because WP-5 did not exist
 * on its branch. It does now, so they are DERIVED: the taxonomy is declared in
 * exactly one place, and drift becomes a compile error at the point a payload
 * is built rather than a divergence a reviewer has to notice.
 *
 * `import type` is erased at build time, so this adds no runtime dependency and
 * no module edge — the contract stays exactly as client-safe as it was.
 */
 ...
 *
 * INTEGRATION: aliases, not restatements. A property added, renamed or retyped
 * in `KlupEventMap` now fails to compile in the builders below — which is the
 * whole reason WP-4a mirrored the shapes in the first place.
 */
