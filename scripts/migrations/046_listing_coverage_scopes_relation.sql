-- 046_listing_coverage_scopes_relation
--
-- Purpose: Replace the overwritable listings.coverage_scope_hash column with the additive listing_coverage_scopes relation; rebuild promote_scrape_run.
-- Depends on: 045
-- Kind: schema migration + function
-- Applied in production: YES (version 20260806132847, applied 2026-08-05/06 via Supabase MCP)
--
...
-- ── SCOPE PROVENANCE MUST BE A RELATION, NOT A COLUMN ───────────────────
-- listings.coverage_scope_hash was overwritten on EVERY promotion, including
-- targeted runs. A `--product=juno-106` run therefore replaced the listing's
-- provenance with a 1-product scope; the full 48-product bootstrap scope
-- would no longer match, and those listings would escape miss accumulation
-- in the established universe permanently.
--
-- The same listing can legitimately be found through several queries and
-- belong to several scopes, so the correct model is a relation.
...
CREATE TABLE listing_coverage_scopes (
  ...
  source_query      text,
  first_seen_run_id uuid REFERENCES scrape_run(id),
  last_seen_run_id  uuid REFERENCES scrape_run(id),
  first_seen_at     timestamptz NOT NULL DEFAULT now(),
  last_seen_at      timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (listing_id, scope_hash)
);

CREATE INDEX idx_lcs_scope  ON listing_coverage_scopes (source, scope_hash);
CREATE INDEX idx_lcs_listing ON listing_coverage_scopes (listing_id);
ALTER TABLE listing_coverage_scopes ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE listing_coverage_scopes IS
  'Which coverage scopes have observed a listing. Membership is ADDITIVE — a targeted run adds its scope but never removes or overwrites membership of the established bootstrap scope. Miss accumulation is restricted to listings that are members of the running scope.';

-- Backfill from the column so existing rows keep their (single) membership.
INSERT INTO listing_coverage_scopes (listing_id, scope_hash, source, source_query)
SELECT id, coverage_scope_hash, source, source_query
FROM listings
WHERE coverage_scope_hash IS NOT NULL
ON CONFLICT DO NOTHING;

COMMENT ON COLUMN listings.coverage_scope_hash IS
  'DEPRECATED — kept for debugging only. Authoritative scope membership lives in listing_coverage_scopes. Never use this column to decide miss eligibility.';

-- ── PROMOTION: additive scope membership, scope-restricted misses ────────
...
BEGIN
  ...
  -- Price events (pre-upsert so "previous price" is the pre-run state).
  WITH staged AS (
  ...
  -- ADDITIVE scope membership: a targeted run adds its own scope but never
  -- removes membership of the established bootstrap scope.
  ...
  ON CONFLICT (listing_id, scope_hash) DO UPDATE
    SET last_seen_run_id = EXCLUDED.last_seen_run_id,
        last_seen_at     = EXCLUDED.last_seen_at,
        source_query     = COALESCE(EXCLUDED.source_query, listing_coverage_scopes.source_query);

  IF p_fail_after_listings THEN
    RAISE EXCEPTION 'injected failure after listings upsert (rollback test)';
  END IF;

  -- Coverage re-evaluated INSIDE this transaction.
  ...
  IF v_lifecycle_ok AND v_scope IS NOT NULL THEN
    ...
    missing AS (
      ...
      FROM listings l
      -- Eligibility comes from the RELATION, not the (overwritable) column.
  ...
  RETURN jsonb_build_object(
    'skipped', false, 'published', v_published, 'first_seen', v_first_seen,
    'price_changes', v_price_changes, 'missed', v_missed, 'delisted', v_delisted,
