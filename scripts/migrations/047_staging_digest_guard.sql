-- 047_staging_digest_guard
--
-- Purpose: Add staging_digest + compute_staging_digest(); reject promotion when staging changed after the gate.
-- Depends on: 046
-- Kind: schema migration + function
-- Applied in production: YES (version 20260806133618, applied 2026-08-05/06 via Supabase MCP)
--
...
-- ── STAGING MUTATION AFTER THE GATE ─────────────────────────────────────
-- The gate evaluates staged rows, then promotion publishes them. If staging
-- changed in between, the data published is NOT the data that was approved.
-- The run records a digest of what was evaluated; promotion recomputes it
-- inside the transaction and refuses on mismatch.

ALTER TABLE scrape_run
  ADD COLUMN IF NOT EXISTS staging_digest text;

COMMENT ON COLUMN scrape_run.staging_digest IS
  'md5 over the staged rows as evaluated by the quality gate. promote_scrape_run recomputes it inside the transaction and aborts on mismatch, so only the exact approved dataset can be published.';

CREATE OR REPLACE FUNCTION compute_staging_digest(p_run_id uuid)
RETURNS text
LANGUAGE sql STABLE
SET search_path = public
AS $$
  SELECT md5(COALESCE(string_agg(
           COALESCE(external_id,'') || '|' || COALESCE(price_dkk::text,'') || '|' ||
           COALESCE(title,'') || '|' || COALESCE(currency,''),
           E'\n' ORDER BY external_id, id), ''))
  FROM listing_staging WHERE run_id = p_run_id;
$$;

-- Add the digest check to promotion (everything else unchanged from 046).
